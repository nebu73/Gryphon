...
def banner (ifc,ip,key):
	...
	print"          :b ` 88888888b`8888 88:88o`Y888				1=> Run Receiver"
	print"          :88boo.Y888888.Y888 88;8888o Y8.			2=> Send a Message"
...
def Conection_Proof(itf,ip,key,typ):
	...
	protos=listdir(getcwd())
	for i in protos:
		if protos[i].startswith("PROTO")==False:
			del protos[i]
			if typ ==0:
				continue
		else:
			...
			os.system("python "+ str(protos[2])+ " 1 " + str(ip)+ " " + key + "abcdefghijk")
			e=open("aux.txt","r")
			proof=e.read()
			if proof==0:
				del protos[i]
...
#Generamos un bucle que va a ir fragmentando el mensaje para su envio
	...
	for a in range (0,count):
		print "Enviando fragmento %s de %s... (%s)"(a +1,count,data[first:last], )
		b=a+1
		payload=key+"_"+b+"_"+count+"_"+data[first:last]
		...
		pkt=os.system("python "+ str(protocolo)+ " 0 " + str(ip)+ " " + payload )
		first+=msfsize
		last+msgsize
